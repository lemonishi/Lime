...
test('eventStartISO uses local time for timed events', () => {
  assert.equal(
    lime.eventStartISO({ allDay: false, startDateTime: '2026-07-30T14:00:00+08:00' }),
    '2026-07-30'
  );
});
...
function timed(startH, endH, summary, day = 30) {
  ...
  return {
    allDay: false,
    summary,
    startDateTime: `2026-07-${p(day)}T${p(startH)}:00:00+08:00`,
    endDateTime: `2026-07-${p(day)}T${p(endH)}:00:00+08:00`,
  };
}
...
Expected: 44 tests passing, and `clean`.
...
In `SETUP.md`, replace the heading line `## M1 acceptance checklist` with `## M1 acceptance checklist` followed by nothing else changed, then append this new section immediately before `## What is deliberately missing in M1`:
...
- [ ] **Step 3: Verify the document is consistent**
...
console.log('secret-URL warning present');
"
```
